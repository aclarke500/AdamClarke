...
const fintech80 = {
  ...
  const smartCartAi = {
    title: 'Smart Cart AI - Fullstack React RAG App',
    description: [
        "Smart Cart AI is a full stack project built using React, Flask, and OpenAI embeddings. It aims to simplify product search, focusing on the semantics within a users query rather than matching key words. It allows users to search for items and look at the most relevant options - even if we don't carry exactly what they are looking for. It is a reactive project built 'mobile first' but works seamlessly across mobile and desktop devies.",
        "For the frontend, the app uses React and is hosted using Netlify. The backend is a REST API built using Flask, and embeddings done with OpenAI's api. It is deployed in a CI/CD pipeline using Render.",
        "SWE best practices were used such as GitHub issues, feature branching, and automated integration tests.",
    ],
    photo: require('@/assets/smartcart.png'),
    githubUrl: "https://github.com/aclarke500/react-rag-store",
    websiteUrl: "https://smartcartai.netlify.app/",
  }
  ...
  const sentenceClassifications = {
    title: 'Sentence Classifications Project',
    description: [
      "This NLP research project investigates the performance of neural networks compared to simpler, more explainable methods such as Euclidean distance for genre classification. Using data from sources like the 'Am I the Asshole' subreddit and the 'Attention is All You Need' paper, the project explores the effectiveness of each approach for classifying sentences by genre.",
...
const projects = reactive({
    ...
    verusProject,
    operatingSystemStudySite,
    classificationProject,
    arduinoTwosComplementProject
})
